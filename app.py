...
# ----------------- ÁREA DO SISTEMA (LOGADO) -----------------
    ...
    if st.session_state["cargo_atual"] == "ADM":
        ...
        # Abas de Navegação do ADM (3 Abas principais controladas sem estouro de índice)
        ...
        with sub_menu_adm[0]:
            ...
            # Filtragem de vales para o período
            ...
            # ----------------- FECHAMENTO FINANCEIRO ORGANIZADO EM 4 COLUNAS -----------------
            ...
            # Nova regra comercial: Aparelhos + Premiações - Vales
            total_bruto_geral = float(total_coletas_bruto) + float(total_premiacoes)
            total_liquido = total_bruto_geral - float(total_vales)
            
            # 4 colunas bem distribuídas para dar espaço físico e o valor NÃO sumir ou cortar em telas menores
            ...
            # Alertas Visuais Inteligentes com base no saldo final líquido
            if total_liquido < 0:
